...
**Achtung beim erneuten Seeden:** Ein einzelner Region-/Reisetipp-Seeder (z. B. um einen Tippfehler oder falsche Koordinaten zu korrigieren) löscht dabei immer auch dessen `Media`-Einträge und erzeugt frische Platzhalter-Illustrationen an denselben Dateipfaden — das setzt bereits per `images:fetch-real` geladene echte Fotos zurück. Nach einem gezielten Seeder-Rerun daher immer direkt `php artisan images:fetch-real --only={slug}` erneut ausführen, um die echten Fotos wiederherzustellen.
