...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-forest-900">Reisetipps</h1>
    ...
    <form method="GET" class="mb-6 flex flex-wrap gap-3">
        <input type="search" name="q" value="{{ request('q') }}" placeholder="Reisetipp suchen…" class="rounded-xl border border-sand-300 py-2 px-4 text-sm w-64">
        <select name="region_id" onchange="this.form.submit()" class="rounded-xl border border-sand-300 py-2 px-4 text-sm">
            <option value="">Alle Regionen</option>
            @foreach ($regions as $region)
                <option value="{{ $region->id }}" @selected(request('region_id') == $region->id)>{{ $region->name }}</option>
            @endforeach
        </select>
        <button type="submit" class="rounded-xl bg-forest-700 hover:bg-forest-800 text-white text-sm font-semibold px-5 py-2">Filtern</button>
    </form>
    ...
    <div class="bg-white rounded-2xl ring-1 ring-sand-200 overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-sand-100">
                @forelse ($tips as $tip)
                    <tr>
                        <td class="px-5 py-3 font-medium text-forest-900">{{ $tip->title }}</td>
                        <td class="px-5 py-3 text-forest-500">{{ $tip->region->name }}</td>
                        <td class="px-5 py-3 text-forest-500">{{ $tip->media_count }}</td>
                        <td class="px-5 py-3">
                            <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $tip->is_published ? 'bg-forest-100 text-forest-700' : 'bg-sand-100 text-sand-700' }}">
                                {{ $tip->is_published ? 'Veröffentlicht' : 'Entwurf' }}
                            </span>
                        </td>
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-8 text-center text-forest-400">Keine Reisetipps gefunden.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
